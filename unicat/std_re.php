...
#customers th {
  ...
  if($_SESSION['urname'] && $_SESSION['type']=="Manager")
  {
	  ...
	  $te_in=mysqli_query($conn,"Select * from regester where type='Parent' and status='auth' and school=(select school from regester where email='$_SESSION[urname]')");
	  //$getinfo=
	  $count=mysqli_num_rows($te_in);
  ...
  <?PHP
  while($getinfo=mysqli_fetch_array($te_in))
  {
	  ?>
	  <tr>
	  <td><?PHP echo $getinfo['Name']; ?></td>
	  <td> <?PHP echo $getinfo['student_ssn']; ?> </td>
