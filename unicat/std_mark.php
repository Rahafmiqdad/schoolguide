...
#customers th {
 ...
 if($_SESSION['urname'] && $_SESSION['type']=="Parent")
 {
	...
	$smark=mysqli_query($dbcon,"Select * from studentm where link=(select ID from regester where email='$_SESSION[urname]')") or die("Err Exec!".mysqli_error($dbcon));
	$nr=mysqli_num_rows($smark);
	if($nr<"1")
	{
		?>
	...
	while($getm=mysqli_fetch_array($smark))
	{
		?>
		<tr>
		<td><?PHP echo $getm['subject']; ?></td>
		<td><?PHP echo $getm['class']; ?></td>
		<td><?PHP echo $getm['result1']; ?></td>
		<td><?PHP echo $getm['result2']; ?></td>
		<td><?PHP echo $getm['result3']; ?></td>
		<td><?PHP echo $getm['result4']; ?></td>
		</tr>
		<?PHP
	}
 }
